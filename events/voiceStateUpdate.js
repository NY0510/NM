...
// Define a constant for the idle time threshold (10 minutes in milliseconds)
const IDLE_TIME_THRESHOLD = 10 * 60 * 1000;

// Create a map to store the last time a user left the voice channel
const lastUserLeftTime = new Map();
...
module.exports = async (client, oldState, newState) => {
	...
	switch (stateChange.type) {
		...
		case "LEAVE":
			if (stateChange.members.size === 0 && !player.paused && player.playing) {
				// Set the time when the last user left the voice channel
				lastUserLeftTime.set(player.voiceChannel, Date.now());

				// Schedule a check after the idle time threshold
				setTimeout(() => {
					// Get the current time
					const currentTime = Date.now();

					// If no users have joined since the last check and idle time is greater than the threshold
					if (stateChange.members.size === 0 && currentTime - lastUserLeftTime.get(player.voiceChannel) >= IDLE_TIME_THRESHOLD) {
						// Send the message and pause the player
						client.channels.cache
							.get(String(player.textChannel))
							.send({ embeds: [new EmbedBuilder().setTitle("⏸️ 음성채널이 비어있어서, 음악을 일시정지 했어요").setColor(process.env.COLOR_NORMAL)] });
